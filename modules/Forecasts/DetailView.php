...
// Push data to the smarty view
$res = $adb->pquery('select * from vtiger_forecasts where forecastsid=?', array($focus->id));
$forecastData = $adb->getNextRow($res, false);
$first_month = $focus->getFirstMonth();
$last_month = $focus->getLastMonth();
$month_data = array();
for ($m=$first_month; $m<=$last_month; $m++) {
	$row = array();
	$row['quota'] = $forecastData['quota_'.$m];
	$row['pcquota'] = $forecastData['pcquota_'.$m];
	$row['closed'] = $forecastData['closed_'.$m];
	$row['committed'] = $forecastData['committed_'.$m];
	$row['bestcase'] = $forecastData['bestcase_'.$m];
	$row['pipeline'] = $forecastData['pipeline_'.$m];
	$month_data[$m] = $row;
}
$smarty->assign('MONTH_DATA', $month_data);

$totals = array(
	'quota' => $forecastData['total_quota'],
	'pcquota' => $forecastData['total_pcquota'],
	'closed' => $forecastData['total_closed'],
	'committed' => $forecastData['total_committed'],
	'bestcase' => $forecastData['total_bestcase'],
	'pipeline' => $forecastData['total_pipeline'],
);
$smarty->assign('TOTALS', $totals);

$smarty->assign('YEAR', $focus->column_fields['year']);
...
$smarty->assign('FCCAT', getTranslatedString($focus->column_fields['productcategory'], 'Products'));
$smarty->assign('funnelVals', $focus->getFunnelValues());
...
if ($singlepane_view == 'true' || $isPresentRelatedListBlock) {
	...
	$index = 1;
	foreach ($related_array as $key => $data) {
		if ($index<$first_month || $index>$last_month) {
