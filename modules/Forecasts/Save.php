...
for ($m=1; $m<=12; $m++) {
	if (!empty($focus->column_fields["quota_{$m}"])) {
		if (empty($focus->column_fields["committed_{$m}"])) {
			$focus->column_fields["committed_{$m}"] = $focus->column_fields["quota_{$m}"];
		}
		if (empty($focus->column_fields["bestcase_{$m}"])) {
			$focus->column_fields["bestcase_{$m}"] = $focus->column_fields["quota_{$m}"];
		}
	} elseif (!empty($focus->column_fields["committed_{$m}"])) {
		$focus->column_fields["quota_{$m}"] = $focus->column_fields["committed_{$m}"];
		if (empty($focus->column_fields["bestcase_{$m}"])) {
			$focus->column_fields["bestcase_{$m}"] = $focus->column_fields["committed_{$m}"];
		}
	} elseif (!empty($focus->column_fields["bestcase_{$m}"])) {
		$focus->column_fields["quota_{$m}"] = $focus->column_fields["bestcase_{$m}"];
		$focus->column_fields["committed_{$m}"] = $focus->column_fields["bestcase_{$m}"];
	}
}
if (!empty($source_id)) {
